...
const pop_rock = new GenerosMusicales("pop rock", [artista1, artista2], [album1, album2], [cancion1, cancion2]);
...
describe('Test para la clase Generos Musicales' , () => {
    ...
    it("Test nombre del género", () => {
        expect(pop_rock.getGenero()).to.be.equal('pop rock');
    });

    it("Test nombre artistas/grupos de ese género ", () => {
        expect(pop_rock.getGruposArtistas()).to.be.eql([artista1, artista2]);
    });

    it("Test nombre albumes de ese género ", () => {
        expect(pop_rock.getGeneroAlbumes()).to.be.eql([album1, album2]);
    });

    it("Test nombre canciones de ese género ", () => {
        expect(pop_rock.getGeneroCanciones()).to.be.eql([cancion1, cancion2]);
    });
